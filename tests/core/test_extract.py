...
class TestCalcExtractedDataset:
    ...
    def test_with_start_end_date_override(self, log_output, monkeypatch):
        ...
        config_yaml = Path("test_extract.yaml")
        start_date, end_date = "2022-06-01", "2022-06-16"

        extracted_ds = extract.calc_extracted_dataset(
            extracted_vars, output_coords, config, config_yaml, start_date, end_date
        )

        assert extracted_ds.name == "test_20220601_20220616"
        assert extracted_ds.attrs["name"] == "test_20220601_20220616"
        assert (
            extracted_ds.attrs["description"]
            == "Day-averaged diatoms biomass extracted from SalishSeaCast v201812 hindcast"
        )
        assert (
            ...
            f"--start-date 2022-06-01 --end-date 2022-06-16`"
        )
        assert extracted_ds.attrs["Conventions"] == "CF-1.6"
