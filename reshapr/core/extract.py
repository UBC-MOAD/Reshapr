...
def calc_extracted_dataset(
    ...
    start_date = (
        arrow.get(cli_start_date) if cli_start_date else arrow.get(config["start date"])
    )
    end_date = (
        arrow.get(cli_end_date) if cli_end_date else arrow.get(config["end date"])
    )
    ...
    if cli_start_date:
        history = f"{history[:-1]} --start-date {cli_start_date}`"
    if cli_end_date:
        history = f"{history[:-1]} --end-date {cli_end_date}`"
